<template>
    <div class="tags-box">
        ...
        <div class="tags" v-if="showTags">
            <ul>
                <li @click="$router.push(item.path)" class="tags-li" v-for="(item,index) in tagsList"
    ...
    export default {
        ...
        methods: {
            ...
            isActive(path) {
                ...
                if(itemPath.indexOf('?') != -1){
                    // console.log(item.path.solit('?'));
                    itemPath=itemPath.split('?')[0];
                }
                if(routeFullPath.indexOf('?') != -1){
                    routeFullPath=routeFullPath.split('?')[0];
                }


                return itemPath === routeFullPath;
            ...
            setTags(route) {
                const isExist = this.tagsList.some(item => {
                    ...
                    let routeFullPath = route.fullPath;
                    if(item.path.indexOf('?') != -1){
                        // console.log(item.path.solit('?'));
                        itemPath=item.path.split('?')[0];
                    }
                    if(route.fullPath.indexOf('?') != -1){
                        routeFullPath=route.fullPath.split('?')[0];
                    }
        ...
        watch: {
            ...
            $route(newValue, oldValue) {
                // console.log('触发了监听路由',newValue);
                this.setTags(newValue);
            }
        },
        created() {
            // console.log('触发了页面初始化');
